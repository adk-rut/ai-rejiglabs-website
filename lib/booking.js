...
import { fetchContact, fetchMessages, threadRows, toHistory } from "./ghl-chat.js";
 ...
 */
export async function bookDiscovery({ contactId, name, email, phone, slot, lang = "en", pageUrl = "", conversationId = "" }) {
  if (!contactId) {
  ...
  const [contact, rows] = await Promise.all([fetchContact(contactId), fetchMessages(conversationId).then(threadRows)]);
  const when = humanSlot(slot, lang);
  const summary = await bookingSummary({ history: toHistory(rows), contact: { ...contact, name: name || contactName(contact), email: email || contact.email, phone: phone || contact.phone }, whenText: humanSlot(slot, "en") });
