...
// Booked state, the note, the card and Beem's offer all say the same thing.
...
async function upsertContact({ name, email, phone, lang, pageUrl, companyName }) {
...
// Work the visitor never waits for (#746). On Vercel the request context's `waitUntil` keeps the
// function alive after the response has gone out, which is what @vercel/functions' waitUntil is
// (five lines around this symbol; not worth a dependency). Anywhere else — tests, a local run —
// there is no context and the work is simply awaited, so the write order stays observable.
async function afterResponse(work) {
  const ctx = globalThis[Symbol.for("@vercel/request-context")]?.get?.();
  const p = work().catch((e) => console.error("[book] after-response work failed", e));
 ...
 */
export async function bookDiscovery({ contactId, name, email, phone, slot, lang = "en", pageUrl = "", conversationId = "", channel = "", answers = {} }) {
  // Which side of the business asked: /blockchain is the Web3 offer, everything else is Rejig.
  const web3 = /\/blockchain(\/|$|\?)/.test(pageUrl);
  if (!contactId) {
    contactId = await upsertContact({ name, email, phone, lang, pageUrl, companyName: answers.Company || "" });
  ...
  if (!appt.ok) {
    const why = await appt.text().catch(() => "");
    console.error("[book] appointment refused", appt.status, why);
    // The contact behind a session token was deleted in GHL (RT clearing test contacts). The
    // token never expires on its own, so this is the one signal the browser gets to drop it.
    if (/contact not found/i.test(why)) return { ok: false, error: "session expired", stale: true };
    return { ok: false, error: "that time is no longer free" };
  }
  const appointmentId = (await appt.json().catch(() => ({})))?.id || "";

  await post(`/contacts/${contactId}/tags`, { tags: ["discovery-booked", `lang-${lang}`, ...(web3 ? ["web3"] : [])] }, CONTACT_VERSION).catch(() => {});

  // Rut's note and card are his, not the visitor's: the visitor's "Booked" goes out now and the
  // summary model call, the note and the ping run after the response (#746 — this second model
  // call was most of the booking turn's 23 s).
  await afterResponse(async () => {
    ...
    const summary = [written, ...answerLines, channel ? `Channel: ${channel}` : "", web3 ? "Channel: Web3 page" : ""].filter(Boolean).join("\n");
  ...
  return { ok: true, contactId, appointmentId, startTime: slot, when: humanSlot(slot, lang) };
}
...
// The two lines Beem adds to her own reply. Code, not model: these carry real times, and the one
