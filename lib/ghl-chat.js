...
// in, post Beem's line back out. Everything channel-specific lives here so `answer-turn.js`
...
// The three channels Beem answers on (#741). `type` is what the messages API takes on a post,
...
// only thing separating a human's line from Beem's — our own posts carry no `userId` either.
...
// The contact's conversation, when the caller has only the contact (#741 follow-up): a GHL Webhook
// action carries `{{contact.id}}` but no reliable conversation merge field, and a conversation is
// per contact, so the newest one IS the DM thread. Best-effort: "" when there is none.
export async function findConversation(contactId) {
  if (!contactId) return "";
  try {
    const r = await fetch(`${GHL_BASE}/conversations/search?locationId=${process.env.GHL_REJIG_LOCATION_ID}&contactId=${contactId}`, { headers: headers() });
    if (!r.ok) return "";
    const j = await r.json();
    return String(j?.conversations?.[0]?.id || "");
  } catch {
    return "";
  }
}
...
// A DM reply goes out as up to three short bubbles with a typing-length pause between them, the way
// a person types (copied from the DuckyCutz relay, never imported — ADR 0014). Paragraphs first;
// a single paragraph splits on sentence ends, where a Latin .!? only counts with whitespace after
// it so a URL's inner dots never split. Past `max` the tail is merged, so nothing is lost.
...
export const bubbleDelayMs = (chunk) => Math.min(2200, Math.max(700, String(chunk).length * 35));
