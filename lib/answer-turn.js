...
// Returns { reply, escalate?, booking? } — `escalate` is 'person' | 'cannot-answer' (#738),
// `booking` is { wants: true, hint } when the visitor asked for the Discovery call (#740).
export async function answerTurn(history = [], text = "", lang = "en") {
  ...
  return { reply: reply || troubleLine(lang), escalate, booking };
}
...
export async function bookingSummary({ history = [], contact = {}, whenText = "" }) {
  ...
  const transcript = history.map((m) => `${m.role === "user" ? "Visitor" : "Jasmin"}: ${m.content}`).join("\n").slice(-4000);
...
3. what price or range Jasmin quoted, and the call time (${whenText})
