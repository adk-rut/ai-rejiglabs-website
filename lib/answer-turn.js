...
// Returns { reply, escalate? } — `escalate` is 'person' | 'cannot-answer' (#738). `booking` joins it in #740.
export async function answerTurn(history = [], text = "", lang = "en") {
  ...
  const reply = stripEmoji(scrub(raw.replace(NO_ANSWER_ALL, "")));
  ...
  return { reply: reply || troubleLine(lang), escalate };
}
