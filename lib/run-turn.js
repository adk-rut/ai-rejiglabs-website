...
import { fetchContact, postInbound, postOutbound, toHistory, channelLabel, splitBubbles, bubbleDelayMs } from "./ghl-chat.js";
...
// Standdown (#730): while Rut's last reply is this fresh he owns the thread and Beem is silent.
...
export async function runTurn({ contactId, conversationId, text, lang = "en", channel = "Live_Chat", rows = [], pageUrl = "" }) {
  ...
  // The widget renders one reply; a DM reads as a person typing, so it goes out in bubbles.
  const bubbles = channel === "Live_Chat" ? [message] : splitBubbles(message);
  for (const [i, bubble] of bubbles.entries()) {
    if (i) await new Promise((r) => setTimeout(r, bubbleDelayMs(bubble)));
    await postOutbound({ contactId, conversationId, message: bubble, type: channel });
  }
