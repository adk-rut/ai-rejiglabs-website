...
export function escalationCard({ reason, contact = {}, pageUrl = "", lang = "en", rows = [], conversationId = "", locationId = "", channel = "", now = Date.now() }) {
  ...
  const who = { visitor: contactName(contact), jasmin: "Beem", rut: "Rut" };
  return [
    // The channel rides on the reason line (#741) so Rut knows where to answer before he reads a
    // word. Empty for the widget: its cards predate the DMs and there is nothing to disambiguate.
    [REASONS[reason] || reason, channel].filter(Boolean).join(" · "),
...
export const standdownCard = ({ contact = {}, text = "", conversationId = "", channel = "" }) =>
  [[`💬 ${contactName(contact)}`, channel].filter(Boolean).join(" · "), text, `ref: ${conversationId}`].join("\n");
