...
test("a /blockchain booking carries its answers into the note and card, and tags the contact web3", async () => {
  ...
  assert.ok(!note.body.body.includes("junk"), note.body.body);
  const tg = fake.calls.find((c) => /telegram/.test(c.url));
  assert.ok(tg.body.text.includes("Building? DeFi"), tg.body.text);
});
